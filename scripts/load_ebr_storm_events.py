#!/usr/bin/env python3
"""One-off LOUISIANA storm_events backfill for the EBR storm-lead arm.

The production loader (app/services/noaa_loader.py) now includes LOUISIANA in
STATE_FILTER, so the nightly cron will keep LA fresh going forward. This script
seeds the initial LA history WITHOUT re-touching the 59K existing TEXAS rows
(box-gentle): it filters NCEI rows to LOUISIANA only and upserts via the same
ON CONFLICT statement the production loader uses.

Peril for EBR is WIND/tropical (Thunderstorm Wind, High/Strong Wind, Tropical
Storm, Hurricane) with Hail secondary — but we load ALL LA event types here;
the peril filtering happens in the unserviced_hail_leads MV, not at load time.

Usage:
    python3 scripts/load_ebr_storm_events.py [--years N]   (default 3)
...
def main():
    ...
    conn.close()
    print(f"DONE: LA storm_events upserted={total} in {time.time()-started:.0f}s",
          flush=True)
    return 0
